...
use crate::world::{Effect, Expr, Identity, Message};
...
pub fn eval_handler(handler: &Expr, payload: &Value, state: &Value) -> Vec<Effect> {
    let mut env = Env::new();
    ...
    match op {
        ...
        "perform" => {
            ...
            match tag {
                ...
                "send" => {
                    if args.len() >= 4 {
                        let to: Identity = eval(&args[1], env).as_str().unwrap_or("").to_string();
                ...
                "remove" => {
                    if args.len() >= 2 {
                        let id: Identity = eval(&args[1], env).as_str().unwrap_or("").to_string();
...
mod tests {
    ...
    #[test]
    fn test_arithmetic() {
        let effects = eval_handler(&val(json!(["+", 1, 2])), &Value::Null, &Value::Null);
    ...
    fn test_set_effect() {
        ...
        let effects = eval_handler(&handler, &Value::Null, &Value::Null);
    ...
    fn test_conditional_handler() {
        ...
        let effects = eval_handler(&handler, &Value::String("open".into()), &Value::Null);
        match &effects[0] {
        ...
        let effects = eval_handler(&handler, &Value::String("close".into()), &Value::Null);
        match &effects[0] {
            Effect::SetState { value, .. } => {
    ...
    fn test_get_in_state() {
        ...
        let effects = eval_handler(&handler, &Value::Null, &state);
